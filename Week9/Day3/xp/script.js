...
const giphyApiUrlSun = "https://api.giphy.com/v1/gifs/search?q=sun&rating=g&api_key=hpvZycW22qCjn5cRM1xtWB8NKq4dQ2My&offset=2&limit=10";

fetch(giphyApiUrlSun)
    .then(response => {
        if (!response.ok) {
            throw new Error(`HTTP error. Status: ${response.status} `);
        }
...
async function fetchStarWars() {
    try {
        ...
        if (!response.ok) {
            throw new Error(`HTTP WError! Status - ${response.status}`);
        }
...
// Output will be immediatly logged 'calling' whithin the asynCall async function 
// and in 2 seconds delay the console will show 'resolved' as a return of resolveAfter2Secons function
